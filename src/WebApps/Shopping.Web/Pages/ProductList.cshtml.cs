    ...
    : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync(string categoryName)
    {
        ...
        CategoryList = response.Value.SelectMany(p => p.Category).Distinct();

        if (!string.IsNullOrWhiteSpace(categoryName))
        {
            ProductList = response.Value.Where(p => p.Category.Contains(categoryName));
    ...
    public async Task<IActionResult> OnPostAddToCartAsync(Guid productId)
    {
        ...
        {
            ProductId = productId,
            ProductName = productResponse.Value.Name,
